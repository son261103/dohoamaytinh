...
GLfloat ambientLightSun[] = { 1.0f, 1.0f, 1.0f, 1.0f }; // Ánh sáng môi trường ban đầu (mặc định là ánh sáng mặt trời)
GLfloat ambientLightMoon[] = { 0.2f, 0.2f, 0.2f, 1.0f }; // Ánh sáng môi trường khi đêm tối hơn
...
void init() {
    glClearColor(1.0f, 0.95f, 0.85f, 1.0f); // Màu nền gần với màu của ánh sáng mặt trời
    glLightfv(GL_LIGHT0, GL_AMBIENT, ambientLightSun); // Kích hoạt ánh sáng môi trường ban đầu
    glEnable(GL_DEPTH_TEST); // Bật kiểm tra chiều sâu
}

void changeBackgroundColor() {
    if (isSunlight) {
        // Nếu màu nền hiện tại là màu tối hơn, thay đổi thành màu ánh sáng mặt trời
        glClearColor(1.0f, 0.95f, 0.85f, 1.0f); // Màu nền gần với màu của ánh sáng mặt trời
        glLightfv(GL_LIGHT0, GL_AMBIENT, ambientLightSun); // Sử dụng biến toàn cục ambientLightSun
    }
    else {
        // Ngược lại, nếu màu nền hiện tại là màu ánh sáng mặt trời, thay đổi thành màu tối hơn
        glClearColor(0.2f, 0.2f, 0.2f, 1.0f); // Màu nền tối hơn
        glLightfv(GL_LIGHT0, GL_AMBIENT, ambientLightMoon); // Sử dụng biến toàn cục ambientLightMoon
    }
...
void keyboard(unsigned char key, int x, int y) {
    ...
    switch (key) {
    case 's': // Khi nhấn phím 's'
        isSunlight = true; // Đặt lại màu nền về mặc định (màu ánh sáng mặt trời)
        changeBackgroundColor(); // Thay đổi màu nền tương ứng
        break;
    case 't': // Khi nhấn phím 't'
        isSunlight = false; // Đặt màu nền thành màu tối hơn
        changeBackgroundColor(); // Thay đổi màu nền tương ứng
        break;
    }
}
